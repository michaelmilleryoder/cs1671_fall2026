...
**Due 2026-09-17, 11:59pm**. *Instructions last updated 2026-09-04.*
...
Use Python 3 and any Python packages you like.
...
We encourage you to use a Python virtual environment to code this and future course assignments. Virtual environments are a way to isolate your code and its dependencies. This is helpful for being able to reproduce the same results and share code with others. 
There are many tools available to help manage Python virtual environments, including [Pixi](http://pixi.sh), which we used in class. See slides from the first few sessions for some useful commands on setting up environments with Pixi.
Feel free to use other tools, such as the [`venv` module](https://docs.python.org/3/library/venv.html)  and the pip package downloader to create and manage virtual environments in Python.
...
* A tutorial on [regular expression lookaheads and lookbehinds](https://www.regular-expressions.info/lookaround.html)
...
* Possible example formats for a color string: `#0b013a`, `#37EfaA`, `rgb(1, 1, 1)`, `rgb(255, 19, 32)`, `rgb(00, 001, 18.0)`, or `rgb(0.1, 0.5, 1.0)`. 
...
For this part, you will use a small data set of wine reviews (see the [Data](#data) section above for download instructions).
You can use `pandas` or some other way to load and manipulate the files.
...
You can choose how you split the text into words (tokenize) or otherwise identify the words in the following questions. It is recommended to use a tokenization tool. Any packages may be imported to aid in tokenization. Take a look at Pandas' [`map`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.map.html) function as one option for applying a tokenizer, but other options are fine, too. It does not matter how efficient your code is.
...
**3.2.** What are the 10 most common words used across all of the reviews, and how many times is each used? Don't include stars in the review, but it is okay to include other punctuation if you like. Keep words in their original case (don't lowercase yet). The most common words can be given in any format (this will be manually graded).
...
* A README.txt file explaining
	1. what version of Python you used
	2. any additional resources, references, or web pages you've consulted
	3. any person with whom you've discussed the assignment and describe the nature of your discussions
	4. any generative AI tool used, and how it was used
	5. any unresolved issues or problems
* A `pyproject.toml`, `requirements.txt` or other file listing all Python packages and their versions in your environment (or on your system if you did not use a virtual environment). 

Please submit this material on Canvas as individual files. If you used Jupyter Notebook to complete the assignment, please download it as a `.py` script to submit.
